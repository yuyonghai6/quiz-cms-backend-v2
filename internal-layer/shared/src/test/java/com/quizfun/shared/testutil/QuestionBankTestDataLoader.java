package com.quizfun.shared.testutil;
...
public class QuestionBankTestDataLoader {
        ...
        try {
            ...
            if (resource.exists()) {
                ...
                for (Object doc : documents) {
                    mongoTemplate.save(doc, "question_banks_per_user");
                }
            }
        } catch (IOException e) {
    ...
    private void loadTaxonomySets() {
        try {
            var resource = new ClassPathResource("test-data/taxonomy-sets.json");
            if (resource.exists()) {
                var documents = objectMapper.readValue(resource.getInputStream(), List.class);
                for (Object doc : documents) {
                    mongoTemplate.save(doc, "taxonomy_sets");
                }
            }
        } catch (IOException e) {
    ...
    private void loadExistingQuestions() {
        try {
            var resource = new ClassPathResource("test-data/existing-questions.json");
            if (resource.exists()) {
                var documents = objectMapper.readValue(resource.getInputStream(), List.class);
                for (Object doc : documents) {
                    mongoTemplate.save(doc, "questions");
                }
            }
        } catch (IOException e) {
    ...
    private void loadExistingQuestionTaxonomyRelationships() {
        try {
            var resource = new ClassPathResource("test-data/existing-question-taxonomy-relationships.json");
            if (resource.exists()) {
                var documents = objectMapper.readValue(resource.getInputStream(), List.class);
                for (Object doc : documents) {
                    mongoTemplate.save(doc, "question_taxonomy_relationships");
                }
            }
        } catch (IOException e) {
